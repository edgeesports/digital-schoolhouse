	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(
			...
			'students' => array('Students', 'This section lists all the students participating in the Digital Schoolhouse esports programme, identified by school and team. Not all roles in the programme are tied to specific teams, so not all students will be members of specific teams.', 'resources/table_icons/user.png', 'None'),
			'teams' => array('Teams', 'This section lists all the teams participating in the Digital Schoolhouse esports programme, identified by school. Anyone can create or join a team, but typically it would be the Team Manager who creates the team and has managing rights over it.', 'resources/table_icons/group.png', 'None'),
			'schools' => array('Schools', 'This section lists all the schools participating in the Digital Schoolhouse esports programme.<br> During the initial phases of the programme you may only see your own school here.', 'resources/table_icons/house.png', 'None'),
			'help' => array('FAQ', 'Help and FAQ for the Digital Schoolhouse esports platform - new users will find here tips and tricks and a guide to getting started.', 'resources/table_icons/accept.png', 'None'),
			'role_types' => array('Role Types', '', 'table.gif', 'None'),
			'roles' => array('Roles', '', 'table.gif', 'None'),
			'games' => array('Games', 'Games currently supported by the Digital Schoolhouse esports programme with PEGI ratings for school and parental control.', 'resources/table_icons/controller.png', 'None'),
			'preliminaries' => array('Preliminaries', 'This section lists the teams participating in the preliminaries to compete in the regional finals.', 'resources/table_icons/traffic_lights_green.png', 'None'),
			'regional_finals' => array('Regional Finals', 'This section lists the teams qualified from the preliminaries to compete for a place at the grand final.', 'resources/table_icons/traffic_lights_yellow.png', 'None'),
			'grand_final' => array('Grand Final', 'This section lists the teams qualified from the regional finals to compete in the grand final.', 'resources/table_icons/traffic_lights_red.png', 'None')
		);
	...
	function get_table_groups($skip_authentication = false){
		...
		$all_groups = array('None', 'mytablegroup');
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'students' => "`students`.`id` as 'id', `students`.`profile_pic` as 'profile_pic', `students`.`first_name` as 'first_name', `students`.`last_name` as 'last_name', `students`.`gamertag` as 'gamertag', `students`.`gender` as 'gender', IF(    CHAR_LENGTH(`schools1`.`school_name`), CONCAT_WS('',   `schools1`.`school_name`), '') as 'school', `students`.`year_group` as 'year_group', `students`.`tutor_group` as 'tutor_group', IF(    CHAR_LENGTH(`role_types1`.`role_type`), CONCAT_WS('',   `role_types1`.`role_type`), '') as 'role_type', IF(    CHAR_LENGTH(`roles1`.`role`), CONCAT_WS('',   `roles1`.`role`), '') as 'role', IF(    CHAR_LENGTH(`teams1`.`team_name`), CONCAT_WS('',   `teams1`.`team_name`), '') as 'team', `students`.`email` as 'email', `students`.`profile_text` as 'profile_text', `students`.`overwatch_stats` as 'overwatch_stats', `students`.`created_by` as 'created_by', `students`.`edited_by` as 'edited_by'",
			'teams' => "`teams`.`id` as 'id', `teams`.`team_logo` as 'team_logo', `teams`.`team_name` as 'team_name', IF(    CHAR_LENGTH(`schools1`.`school_name`), CONCAT_WS('',   `schools1`.`school_name`), '') as 'school', `teams`.`matches` as 'matches', `teams`.`wins` as 'wins', `teams`.`losses` as 'losses', `teams`.`draws` as 'draws', `teams`.`points` as 'points', `teams`.`created_by` as 'created_by', `teams`.`edited_by` as 'edited_by'",
			'schools' => "`schools`.`id` as 'id', `schools`.`school_crest` as 'school_crest', `schools`.`school_name` as 'school_name', `schools`.`school_address` as 'school_address', `schools`.`school_postcode` as 'school_postcode', `schools`.`contact_name` as 'contact_name', `schools`.`contact_email` as 'contact_email', `schools`.`contact_phone` as 'contact_phone', `schools`.`esports_coach` as 'esports_coach', `schools`.`created_by` as 'created_by', `schools`.`edited_by` as 'edited_by'",
			'help' => "`help`.`id` as 'id', `help`.`faq` as 'faq'",
			'role_types' => "`role_types`.`id` as 'id', `role_types`.`role_type` as 'role_type', `role_types`.`created_by` as 'created_by', `role_types`.`edited_by` as 'edited_by'",
			'roles' => "`roles`.`id` as 'id', `roles`.`role` as 'role', IF(    CHAR_LENGTH(`role_types1`.`role_type`), CONCAT_WS('',   `role_types1`.`role_type`), '') as 'role_type', `roles`.`created_by` as 'created_by', `roles`.`edited_by` as 'edited_by'",
			'games' => "`games`.`id` as 'id', `games`.`game_logo` as 'game_logo', `games`.`game_title` as 'game_title', `games`.`pegi_rating` as 'pegi_rating', `games`.`created_by` as 'created_by', `games`.`edited_by` as 'edited_by'",
			'preliminaries' => "`preliminaries`.`id` as 'id', `preliminaries`.`team` as 'team', `preliminaries`.`matches` as 'matches', `preliminaries`.`wins` as 'wins', `preliminaries`.`losses` as 'losses', `preliminaries`.`draws` as 'draws', `preliminaries`.`points` as 'points', `preliminaries`.`created_by` as 'created_by', `preliminaries`.`edited_by` as 'edited_by'",
			'regional_finals' => "`regional_finals`.`id` as 'id', `regional_finals`.`team` as 'team', `regional_finals`.`matches` as 'matches', `regional_finals`.`wins` as 'wins', `regional_finals`.`losses` as 'losses', `regional_finals`.`draws` as 'draws', `regional_finals`.`points` as 'points', `regional_finals`.`created_by` as 'created_by', `regional_finals`.`edited_by` as 'edited_by'",
			'grand_final' => "`grand_final`.`id` as 'id', `grand_final`.`team` as 'team', `grand_final`.`matches` as 'matches', `grand_final`.`wins` as 'wins', `grand_final`.`losses` as 'losses', `grand_final`.`draws` as 'draws', `grand_final`.`points` as 'points', `grand_final`.`created_by` as 'created_by', `grand_final`.`edited_by` as 'edited_by'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'students' => "`students` LEFT JOIN `schools` as schools1 ON `schools1`.`id`=`students`.`school` LEFT JOIN `role_types` as role_types1 ON `role_types1`.`id`=`students`.`role_type` LEFT JOIN `roles` as roles1 ON `roles1`.`id`=`students`.`role` LEFT JOIN `teams` as teams1 ON `teams1`.`id`=`students`.`team` ",
			'teams' => "`teams` LEFT JOIN `schools` as schools1 ON `schools1`.`id`=`teams`.`school` ",
			'schools' => "`schools` ",
			'help' => "`help` ",
			'role_types' => "`role_types` ",
			'roles' => "`roles` LEFT JOIN `role_types` as role_types1 ON `role_types1`.`id`=`roles`.`role_type` ",
			'games' => "`games` ",
			'preliminaries' => "`preliminaries` ",
			'regional_finals' => "`regional_finals` ",
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Digital_Schoolhouse_powered_by_PlayStation_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Digital_Schoolhouse_powered_by_PlayStation_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Digital_Schoolhouse_powered_by_PlayStation_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Digital_Schoolhouse_powered_by_PlayStation_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Digital Schoolhouse powered by PlayStation</a>
			</div>
	...
	*/
	...
	function get_embed_googlemap($url, $max_width = '', $max_height = '', $retrieve = 'html'){
		...
		if(preg_match($coords_regex, $url_parts['path'] . '?' . $url_parts['query'], $m)){
			...
			$api_key = 'AIzaSyD1tffn7vzW4GJBqlBfjU3CqQzXu1u6WSs';
	...
	function NavMenus($options = array()){
		...
		if(is_array($arrTables)){
			foreach($arrTables as $tn => $tc){
				...
				$tChkHL = array_search($tn, array('role_types','roles','games','preliminaries','regional_finals','grand_final'));
...
EOT;
	...
	function StyleSheet(){
		...
		$css_links = <<<EOT
			...
			<link rel="stylesheet" href="{$prepend_path}resources/initializr/css/edge.css">
